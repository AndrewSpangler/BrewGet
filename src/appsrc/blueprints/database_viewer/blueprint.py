...
@app.permission_required(app.models.user.PermissionsLevel.ADMIN)
def view(database=None, table=None, uid=None):
    bind_lookup = {
        os.path.abspath(v): k
        for k, v in app.config["SQLALCHEMY_BINDS"].items()
    }       
    databases = {v:[] for k,v in bind_lookup.items()}
    engines, table_to_engine = {}, {}
    for table_object, engine in db.get_binds().items():
        bind = bind_lookup[os.path.normpath(os.path.abspath(str(engine.url)))]
